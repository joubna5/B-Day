...
class PricingCalculator {
    constructor() {
        this.basePrices = {
            ...
            extraPage: 3000,        // Halaman 5 dan seterusnya +3k
...
`).join('');
    ...
    calculatePrice(currentSettings) {
        ...
        if (currentSettings.enableBook) {
            ...
            // Kiểm tra số trang thêm (>3 trang)
            const totalPages = currentSettings.pages ? currentSettings.pages.length : 0;
            if (totalPages > 4) {  // ubah dari 10 menjadi 4
                const extraPages = totalPages - 4;  // hitung halaman ekstra setelah 4
                const extraPagesCost = extraPages * this.basePrices.extraPage;
                items.push({
                    name: `Halaman tambahan (${extraPages} halaman)`,
        ...
        if (currentSettings.isSave) {
            items.push({
                name: 'Simpan QR secara permanen',
    ...
    async handleActionButton() {
        ...
        try {
            ...
            if (selectedPaymentMethod === 'POLAR') {
                ...
                if (totalUSD < 1) {
                    this.showNotification('❌ Jumlah pembayaran minimum adalah 1 USD!', 'error');
                    return;
                }
            } else if (this.currentTotal > 0 && this.currentTotal < 10000) {
                this.showNotification('❌ Jumlah pembayaran minimum adalah 10.000 Rupiah!', 'error');
                return;
            }

            // ✅ Phân biệt Free vs Paid dựa trên giá tiền
            if (this.currentTotal > 0) {
                // Ada harga - proses pembayaran
                actionButton.textContent = '⏳ Sedang memproses pembayaran...';
                await this.handlePaidCreation();
            } else {
                // Gratis - buat langsung
                actionButton.textContent = '⏳ Sedang membuat...';
                await this.handleFreeCreation();
            }
        } catch (error) {
            console.error('Error di handleActionButton:', error);
        ...
        try {
            ...
            if (settingsToUse.enableBook) {
                ...
                if (totalPages > 1 && totalPages % 2 === 0) {
                    this.showNotification(`❌ Struktur halaman tidak valid! Saat ini ada ${totalPages} halaman. Silakan tambah atau hapus 1 halaman agar struktur valid.`, 'error');
                    return;
                }
            }
            ...
            if (settingsToUse.enableBook && settingsToUse.pages?.length > 0) {
                progressNotification = this.showProgressNotification('📤 Sedang mengunggah gambar halaman buku...');
            }
            ...
            if (progressNotification) {
                this.updateProgressNotification(progressNotification, '🌐 Sedang membuat website...');
            }
            ...
            if (!websiteResult.success) {
                throw new Error(websiteResult.error || 'Tidak dapat membuat website');
            }

            // Tạo sản phẩm
            if (progressNotification) {
                this.updateProgressNotification(progressNotification, '📦 Sedang membuat produk...');
            }
            ...
            if (this.selectedVoucher) {
                if (progressNotification) {
                    this.updateProgressNotification(progressNotification, '🎫 Sedang menerapkan voucher...');
                }
            ...
            this.showNotification('❌ Terjadi kesalahan saat membuat website: ' + error.message, 'error');
        }
    }
        ...
        try {
            ...
            if (this.selectedVoucher) {
                ...
                try {
                    ...
                    this.showNotification('⚠️ Gagal menerapkan voucher, lanjutkan pembayaran dengan harga asli', 'warning');
                }
            }
            ...
            this.showNotification('❌ Terjadi kesalahan saat melakukan pembayaran: ' + error.message, 'error');
        }
    }
    ...
    prepareCompleteSettings(settingsToUse) {
        ...
        const completeSettings = {
            music: settingsToUse.music || './music/happybirthday.mp3',
    ...
    async createProduct(productData) {
        try {
            ...
            if (data.success) {
                ...
                throw new Error(data.message || 'Terjadi kesalahan saat membuat produk');
            }
        } catch (error) {
            throw new Error('Terjadi kesalahan saat membuat produk: ' + error.message);
        }
    }
    ...
    async applyVoucher(orderCode) {
        try {
            ...
            if (!voucherData.success) {
                throw new Error(voucherData.message || 'Gagal menerapkan voucher!');
            }
    ...
    async processPayment(orderCode, amount, websiteId, paymentMethod = 'PAYOS') {
        try {
            ...
            throw new Error('Terjadi kesalahan saat memproses pembayaran: ' + error.message);
        }
    }
    ...
    showPaymentModal(checkoutUrl, websiteId, amount) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (window.socketManager && window.socketManager.isSocketConnected()) {
                ...
                const paymentStatusHandler = (data) => {
                    if (String(data.orderCode) === String(orderCode)) {
                        ...
                        if (data.status === 'PAID') {
                            ...
                            window.socketManager.off('payment_status_update', paymentStatusHandler);
                            reject(new Error(data.message || 'Pembayaran gagal!'));
                        }
                    } else {
                    }
                };
                ...
                console.error('❌ WebSocket belum dibuat!');
            ...
            setTimeout(() => {
                if (!wsMessageReceived) {
                    window.cleanupPaymentState(orderCode, false);
                    this.showNotification('Timeout - Tidak dapat menentukan status pembayaran!', 'warning');
                    reject(new Error('Timeout - Tidak dapat menentukan status pembayaran!'));
                }
            }, 300000); // 5 phút
        });
    }
        ...
        if (shareURL) {
            ...
            try {
                navigator.clipboard.writeText(shareURL.value).then(() => {
                    copyButton.textContent = '✅ Sudah disalin!';
                    this.showNotification('📋 Link sudah disalin ke clipboard!', 'success');
                    setTimeout(() => {
                ...
                this.showNotification('❌ Tidak dapat menyalin link!', 'error');
            }
        }
    }
