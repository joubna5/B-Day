...
const musicOptions = [
    { value: './music/happybirthday.mp3', label: 'Happy Birthday Song' },
    { value: './music/oldlove.mp3', label: 'Old Love' },
    { value: './music/perfect.mp3', label: 'Perfect' },
    { value: './music/herewithme.mp3', label: 'Here With Me' },
    { value: './music/givemeyourforever.mp3', label: 'Give Me Your Forever' },
    { value: './music/wannabeyours.mp3', label: 'Wanna Be Yours' },
    { value: './music/stuckwithyou.mp3', label: 'Stuck With You' },
    { value: './music/untilifoundyou.mp3', label: 'Until I Found You' },
    { value: './music/sombadypleasure.mp3', label: 'Sombady Pleasure' },
    { value: './music/dandelions.mp3', label: 'Dandelions' },
    { value: './music/gluesong.mp3', label: 'Glue Song' }
];
const gifOptions = [
    { value: '', label: 'None' },
    { value: './gif/hbd1.gif', label: 'Hbd Gif1' },
    { value: './gif/hbd2.gif', label: 'Hbd Gif2' },
    { value: './gif/hbd3.gif', label: 'Hbd Gif3' },
    { value: './gif/sorry.gif', label: 'Sorry Gif' },
    { value: './gif/love.gif', label: 'Love Gif' }
];
...
function getIdlePreviewMessage() {
    ...
    return label ? `Selected: ${label}` : 'Select a song and press "Play Song"';
}

function setMusicPreviewState({ message, isPlaying }) {
    if (musicPreviewButton) {
        musicPreviewButton.textContent = isPlaying ? '⏸ Pause' : '▶ Play Song';
        if (isPlaying) {
...
function handleMusicPreview() {
    ...
    if (!musicSelect || !musicSelect.value) {
        setMusicPreviewState({
            message: 'Vui lòng chọn một bài nhạc trước khi Play Song',
            isPlaying: false
        });
        return;
    }
    ...
    musicPreviewAudio.play().then(() => {
        ...
        stopMusicPreview('Không phát được bản Play Song. Vui lòng thử lại.');
    });
}
...
function resetWebsiteState() {
    ...
    // CHANGED
    // ✅ Update pages and recreate book
    if (window.settings && window.settings.pages) {
        // Sisipkan cover depan secara virtual agar struktur array sama persis dengan input manual
        pages = [
...
// }
        ...
        if (page.image) {
            ...
            const placeholderText = t('pagePlaceholder', {num: index + 1});
            imagePreview.style.cssText += `
                display: flex;
...
function createPages() {
    ...
    for (let physicalPageIndex = 0; physicalPageIndex < totalPhysicalPages; physicalPageIndex++) {
        ...
        if (backLogicalIndex < pages.length && pages[backLogicalIndex]) {
            ...
            // COVER BELAKANG OTOMATIS JIKA HABIS
            const endImg = document.createElement('img');
            endImg.src = './image/theend.jpg';
            endImg.onerror = function () {
                back.classList.add('empty-page');
...
if (languageSwitchBtn) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            window.settings = {
                music: serverSettings.music || './music/happybirthday.mp3',
...
applySettingsButton.addEventListener('click', () => {
    ...
    // ✅ Karena Cover sudah fix, input sekarang berpasangan (kiri & kanan). Jadi harus bernilai genap.
